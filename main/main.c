...
static int blecent_on_write(uint16_t conn_handle, const struct ble_gatt_error *error, struct ble_gatt_attr *attr, void *arg)
{
    ...
    if (dsc == NULL)
    {
        MODLOG_DFLT(ERROR, "Error: Peer lacks a CCCD for the Unread Alert Status characteristic\n");
        goto err;
    }
...
static int blecent_on_read(uint16_t conn_handle, const struct ble_gatt_error *error, struct ble_gatt_attr *attr, void *arg)
{
    ...
    if (chr == NULL)
    {
        MODLOG_DFLT(ERROR, "Error: Peer doesn't support the Alert Notification Control Point characteristic\n");
        goto err;
    }
...
static void blecent_read_write_subscribe(const struct peer *peer)
{
    ...
    if (chr == NULL)
    {
        MODLOG_DFLT(ERROR, "Error: Peer doesn't support the Supported New Alert Category characteristic\n");
        goto err;
    }
...
static void blecent_connect_if_interesting(const struct ble_gap_disc_desc *disc)
{
    ...
    rc = ble_gap_connect(BLE_OWN_ADDR_PUBLIC, &disc->addr, 30000, NULL, blecent_gap_event, NULL);
    if (rc != 0)
    {
        MODLOG_DFLT(ERROR, "Error: Failed to connect to device; addr_type=%d addr=%s\n", disc->addr.type, addr_str(disc->addr.val));
        return;
    }
}

#define MAC2STR_REVERSE(a) (a)[5], (a)[4], (a)[3], (a)[2], (a)[1], (a)[0]
#define MACSTR_LOCAL "%02x:%02x:%02x:%02x:%02x:%02x"

void print_buf_hex(uint8_t *buf, size_t len, uint8_t *mac, uint8_t type)
{
    char *name = NULL;
    bool onlyLE;
    bool discoverable;
    ...
    if (adv_ind)
    {
        ...
        {
            ESP_LOGI(tag, "MAC: " MACSTR_LOCAL ", Discoverable: %s, Mode: %s, Data: ", MAC2STR_REVERSE(mac), discoverable ? "Discoverable" : "Not discoverable", onlyLE ? "Only LE" : "LE & BR/EDR");
            for (int i = 0; i < len; i++)
                printf("%02x ", buf[i]);
        }
    }
    ...
    //If scan response data
    {
        if (buf[1] == 0x09)
        {
            if (buf[0] > 0)
            {
                name = pvPortMalloc((size_t)buf[0]);
                memcpy(name, buf + 2, (size_t)buf[0] - 1);
                name[(size_t)buf[0] - 1] = '\0';
                ESP_LOGI(tag, "MAC: " MACSTR_LOCAL ", Name: %s", MAC2STR_REVERSE(mac), name);
                safe_free(name);
            }
        }
    }
...
static int blecent_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC:
        print_buf_hex(event->disc.data, event->disc.length_data, (uint8_t *)&event->disc.addr.val, event->disc.event_type);
